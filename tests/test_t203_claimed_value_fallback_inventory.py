...
def current_public_route(program: Program, initial_variables: jit.InitialVariables | None = None) -> str:
    ...
    if jit.requires_host_runtime_execution(program):
        return "rejected"
    if (
        (jit.requires_host_runtime_value_execution(program) and not string_initial_variables)
...
def test_t203_matrix_records_the_representative_claimed_value_fallback_rows() -> None:
    ...
    assert "| Unset scalar string-context read | `BEGIN { print x }` | none | yes | yes | `requires_host_runtime_value_execution(program) == True` | yes |" in matrix_text
    assert "| Unset scalar propagation through assignment | `BEGIN { y = x; print y }` | none | yes | yes | `requires_host_runtime_value_execution(program) == True` | no |" in matrix_text
    assert "| Mixed unset-scalar string and numeric views | `BEGIN { print x; print x + 1 }` | none | yes | yes | `requires_host_runtime_value_execution(program) == True` | yes |" in matrix_text
    assert "| Plain scalar-name read after assignment | `BEGIN { x = 1; print x }` | none | yes | yes | `requires_host_runtime_value_execution(program) == True` | yes |" in matrix_text
...
def test_t203_inventory_rows_match_the_current_value_fallback_boundary() -> None:
    ...
    assert current_public_route(unset_print) == "host"
    assert current_public_route(unset_assignment) == "host"
    assert current_public_route(mixed_views) == "host"
    assert current_public_route(assigned_print) == "host"
