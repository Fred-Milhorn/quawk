...
def test_t157_spec_splits_output_surface_rows() -> None:
    ...
    assert "| Bare `print` / implicit `$0` | partial |" in spec_text
    assert "| Multi-argument `print` | partial |" in spec_text
    assert "| `OFS` / `ORS` driven print behavior | partial |" in spec_text
    assert "| `printf` basic execution | implemented |" in spec_text
    assert "| Full POSIX `printf` parity | partial |" in spec_text
    assert "| Output redirection and pipe output | planned |" in spec_text
...
def test_t157_spec_splits_builtin_and_backend_rows() -> None:
    ...
    assert "| Output and formatting builtin variables | partial |" in spec_text
