...
def test_t200_matrix_records_the_current_residual_classifications() -> None:
    ...
    assert "| Logical-or | `BEGIN { print 1 || 0 }` | yes | yes | yes | no | no | no | unclaimed and backend-incomplete |" in matrix_text
    assert "| Broader comparisons | `BEGIN { print 1 != 0 }` | yes | yes | yes | no | no | no | unclaimed and backend-incomplete |" in matrix_text
    assert "| Broader arithmetic | `BEGIN { print 6 / 2 }` | yes | yes | yes | no | no | no | unclaimed and backend-incomplete |" in matrix_text
    assert "| Ternary | `BEGIN { print (1 ? 2 : 3) }` | yes | yes | yes | no | no | no | unclaimed and backend-incomplete |" in matrix_text
    assert '| Match operators | `BEGIN { print ("abc" ~ /b/) }` | yes | yes | yes | no | no | no | unclaimed and backend-incomplete |' in matrix_text
    assert '| `in` | `BEGIN { a["x"] = 1; print ("x" in a) }` | yes | yes | yes | no | no | no | unclaimed and backend-incomplete |' in matrix_text
