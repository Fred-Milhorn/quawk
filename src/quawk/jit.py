...
def supports_runtime_backend_subset(program: Program) -> bool:
    ...
    # With the standalone direct lane removed, this helper now classifies the
    # full reusable compiled surface rather than only programs that happened to
    # need an earlier runtime-specific routing path.
    for item in program.items:
        if isinstance(item, FunctionDef):
