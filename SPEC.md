...
| Backend parity for broader frontend-admitted POSIX forms | partial | The remaining product-side forms outside the current AOT-backed contract are explicit: broader `sub()` / `gsub()` target shapes outside the currently admitted lvalue subset; builtin names beyond the current subset; top-level items outside `PatternAction` / `FunctionDef`; and retirement of the narrow direct-function lane as internal debt. Newly claimed widening waves are only rebaselined once ordinary public execution, `--ir`, and `--asm` all stay on the compiled backend/runtime path with no public Python host fallback. |
