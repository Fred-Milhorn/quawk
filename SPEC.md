...
| Remaining POSIX compatibility corroboration gaps | implemented | The remaining compatibility-only closeout item has been audited and closed out: `rand()` is direct-test-only under the checked-in reference-disagreement policy, and no stale reviewed gaps remain. |
