...
| Remaining parser-admitted execution gaps | planned | The remaining product-side end-to-end gaps are now explicit: compound assignment, parser-admitted non-name iterable or RHS forms for `for ... in` and `in`, non-name `split()` targets, broader `sub()` / `gsub()` target shapes, builtin names beyond the current claimed subset, top-level items outside `PatternAction` / `FunctionDef`, and retirement of the narrow direct-function execution lane. |
...
| Broader builtin-name inventory beyond the current subset | planned | Builtin names outside the current claimed subset still need explicit POSIX-vs-extension classification before the public contract widens again. |
