...
| Expressions | partial | The currently claimed AOT-backed subset includes `+`, `-`, `*`, `/`, `%`, `^`, `<`, `<=`, `>`, `>=`, `==`, `!=`, `&&`, `||`, `~`, `!~`, ternary expressions over the current claimed numeric/string subset, `in`, concatenation, unary `+`/`-`/`!`, pre/post increment and decrement, and plain assignment expressions. Broader parser-admitted corners still remain intentionally outside the current claim, including compound assignment and other forms now tracked explicitly in the remaining-gap rows below. |
| Remaining parser-admitted execution gaps | planned | The `T-272` baseline classifies the remaining product-side gaps explicitly: compound assignment is the only checked-in POSIX-required execution gap; parser-admitted non-name iterable or RHS forms for `for ... in` and `in`, non-name `split()` targets, and top-level items outside `PatternAction` / `FunctionDef` are intentionally out of contract; broader `sub()` / `gsub()` targets split between POSIX-required array-element lvalues and out-of-contract non-lvalue expressions; and the narrow direct-function execution lane is internal debt rather than public surface. |
...
| Backend parity for broader frontend-admitted POSIX forms | partial | The remaining product-side forms outside the current AOT-backed contract are explicit: compound assignment; parser-admitted non-name iterable or RHS forms for `for ... in` and `in`; non-name `split()` targets; broader `sub()` / `gsub()` target shapes outside the currently admitted lvalue subset; builtin names beyond the current subset; top-level items outside `PatternAction` / `FunctionDef`; and retirement of the narrow direct-function lane as internal debt. Newly claimed widening waves are only rebaselined once ordinary public execution, `--ir`, and `--asm` all stay on the compiled backend/runtime path with no public Python host fallback. |
| Remaining POSIX compatibility corroboration gaps | planned | The remaining compatibility-only closeout items are explicit: re-audit the field-rebuild corroborating anchors, resolve the reviewed record-target `gsub` skip, and decide the final `rand()` corroboration or reference-disagreement policy. |
