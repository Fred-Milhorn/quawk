...
1. `T-114` complete semantic validation for the remaining POSIX-core syntax surface
2. `T-115` author runtime and builtin baselines for the remaining POSIX-core execution surface
3. `T-116` replace the scalar runtime model with AWK-style value cells and coercions
4. `T-117` implement the remaining POSIX-core runtime semantics for statements, patterns, records, and fields
5. `T-118` implement the remaining POSIX-core builtins and builtin variables
6. `T-119` author backend-parity and inspection baselines for the completed POSIX-core subset
7. `T-120` extend lowering and runtime ABI coverage to the completed POSIX-core subset
8. `T-121` remove host-runtime-only status for supported POSIX-core features and close pre-compatibility backend gaps
