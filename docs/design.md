...
- the currently claimed expression subset: `+`, `-`, `*`, `/`, `%`, `^`, `<`, `<=`, `>`, `>=`, `==`, `!=`, `&&`, `||`, pure ternary expressions over the current claimed numeric/string subset, concatenation, unary `+`/`-`/`!`, pre/post increment and decrement, and plain assignment expressions
...
- broader frontend-admitted but not yet claimed POSIX forms, such as match operators and `in`, still sit outside the current AOT-backed contract
