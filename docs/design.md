...
- bare `print`, single-argument `print`, multi-argument `print`, `printf`, and `OFS` / `ORS`-driven print output
- field reads, dynamic field assignment, and builtin variables such as `NR`, `FNR`, `NF`, `FILENAME`, `OFS`, and `ORS`
