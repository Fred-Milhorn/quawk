...
This file preserves the prior full roadmap text, detailed phase writeups, and
historical task ledger.

Use [docs/roadmap.md](roadmap.md) for the current live roadmap and active
backlog. Some items in this archive may appear open because they reflect older
planning snapshots or superseded backlog entries rather than the current live
...
- reclassify the `corpus` CLI as a manual harness tool rather than a primary
  test gate
- implementation details for this phase are reflected directly in `docs/testing.md`
...
- implementation details for this phase are reflected directly in the current compatibility docs and package layout
...
- make an explicit product decision about whether ordinary `quawk` should keep
  ...
  AOT-backed contract
- implementation details for this phase are reflected directly in `SPEC.md`,
  `docs/design.md`, and `POSIX.md`
...
- implementation details live in
  [plans/performance-implementation.md](plans/performance-implementation.md)
...
- implementation details for this phase are reflected directly in
  `docs/design.md`, `SPEC.md`, and `docs/plans/backend-gap-inventory.md`
...
- implementation details for this phase are reflected directly in `SPEC.md` and
...
- implementation details for this phase are reflected directly in
  `docs/design.md` and `docs/plans/backend-gap-inventory.md`
...
- tighten git and documentation hygiene guidance for dirty worktrees, staging,
  and relative links
- implementation details for this phase are reflected directly in `AGENTS.md`
  and the workflow docs it names
...
- implementation details for this phase are reflected directly in
  `src/quawk/README.md` and the current source layout
...
- see `docs/quawk.ebnf`, `docs/quawk.asdl`, and
...
| T-178 | P16 | P0 | Author the testing-surface rename and consolidation baseline | T-177 | `docs/testing.md` and focused regression tests make the current marker names, command surfaces, and overlap explicit before implementation | done |
...
| T-188 | P17 | P1 | Rebaseline repo layout docs and final namespace audit after the refactor lands | T-187 | Roadmap/docs and focused compatibility-tooling regressions agree on the final layout, and no stale flat-module or wrapper-script references remain | done |
...
| T-197 | P19 | P0 | Author the residual host-runtime boundary audit baseline and scope | T-192 | `POSIX.md`, `SPEC.md`, and the roadmap make the backend-first purpose, audit scope, and required outputs explicit before new implementation decisions start | done |
...
| T-227 | P25 | P0 | Design slot allocation data structures for compile-time variable slots | T-226 | `VariableSlot` and `SlotAllocation` structs are defined and reviewed in `docs/plans/performance-implementation.md` | done |
...
| T-296 | P35 | P0 | Author the agent-workflow documentation alignment plan | T-295 | `AGENTS.md` and the touched workflow docs now record the missing guidance, desired steady state, validation direction, and acceptance criteria before docs are rewritten | done |
...
| T-302 | P36 | P0 | Author the implementation readability refactor plan | T-301 | The roadmap and source-level implementation map record the hotspots, proposed phases, guardrails, validation commands, and definition of done before code moves begin | done |
...
| T-312 | P37 | P0 | Author the parser/EBNF conformance plan | T-311 | The roadmap and parser/EBNF inventory record the parser-only scope, coverage-inventory approach, fixture expansion, edge-case testing, validation, and definition of done | done |
